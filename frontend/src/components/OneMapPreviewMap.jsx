  ...
  if (!leafletLoadPromise) {
    leafletLoadPromise = new Promise((resolve, reject) => {
      ...
      if (existingScript) {
        ...
        existingScript.addEventListener('error', () => reject(new Error('Failed to load OneMap Leaflet script.')), {
          once: true
        });
        return;
      }
      ...
      script.onerror = () => reject(new Error('Failed to load OneMap Leaflet script.'));
...
function toneColor(tone) {
  if (tone === 'critical' || tone === 'hazard' || tone === 'danger') {
    return '#c5162b';
  }

  if (tone === 'warning' || tone === 'high' || tone === 'restricted') {
    return '#d9821d';
  }

  if (tone === 'safe' || tone === 'support' || tone === 'success') {
    return '#0d9464';
  }
...
async function resolvePoint(point) {
  ...
  try {
    const response = await fetchJson(`/api/v1/onemap/search?query=${encodeURIComponent(point.query)}`);
    ...
    // Fall back to provided coordinates when OneMap search is unavailable.
  }

  return typeof point.fallbackLatitude === 'number' && typeof point.fallbackLongitude === 'number'
    ? {
        ...point,
        latitude: point.fallbackLatitude,
        longitude: point.fallbackLongitude
      }
    : null;
}
...
export function OneMapPreviewMap({ className = '', points = [], zoom = 12 }) {
  ...
  useEffect(() => {
    ...
    async function setupMap() {
      try {
        ...
        if (isCancelled || !mapRef.current || leafletMapRef.current) {
          return;
        }
  ...
  useEffect(() => {
    ...
    async function syncPoints() {
      if (!leafletMapRef.current || !overlayLayerRef.current) {
        return;
      }
      ...
      if (isCancelled || !overlayLayerRef.current || !leafletMapRef.current) {
        return;
      }
      ...
      resolved.forEach((point) => {
        ...
        L.circleMarker(latLng, {
          ...
          .bindPopup(
            `<div class="one-map-popup"><strong>${point.title ?? point.label ?? point.query ?? 'Location'}</strong><p>${
      ...
      leafletMapRef.current.fitBounds(bounds.pad(0.18));
    }
